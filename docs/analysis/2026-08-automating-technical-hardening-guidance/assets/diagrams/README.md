...
Nine SVG files and one small script. The library builds nineteen; tools/diagrams.py writes only the nine the site shows, listed in its PUBLISHED constant. Every one of them is generated by
`tools/diagrams.py` from `data/`, and `tools/verify.py --diagrams` re-derives
them and checks the result. **Do not hand-edit an `.svg` in this folder.** The
...
Two marks are local extensions, and both are declared in the legend of every
figure that uses them, because neither had a place in the vocabulary and neither
overloads an existing one:

| Mark | Means | Where |
|---|---|---|
| plain elbow, no head | containment: the child sits inside the parent | 72, 73 |
| socket, hollow with two seating tabs | a slot that may legitimately be published empty | 77, 79 |
...
<figure>
  <figcaption>
    The seven OSCAL models across three layers. After Figure 1 of the pre-read.
  </figcaption>
</figure>
```
...
scale figure. The figure is complete without it: all three destinations and all
three resulting counts are printed in the SVG, so a reader with no scripting
loses the animation and no information.
...
2. **Write a `diagram_NN(c)` function** in `tools/diagrams.py` returning
   ...
   from the `Corpus` object, and put every field name through `F()`.
...
|---|---|---|
| `71-layer-map-assessment.svg` | 7.1 | Assessment-first footprint |
| `71-layer-map-catalog.svg` | 7.1 | Catalog-first footprint |
| `71-layer-map-component.svg` | 7.1 | Component-first footprint |
| `71-layer-map-overlay.svg` | 7.1 | all three, nested |
...
| `75-stakeholders-*.svg` | 7.5 | which stakeholder each approach serves first |
